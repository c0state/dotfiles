...
# init tailscale
curl -fsSL "https://pkgs.tailscale.com/stable/ubuntu/$(source /etc/os-release && echo "$VERSION_CODENAME").noarmor.gpg" | sudo tee /usr/share/keyrings/tailscale-archive-keyring.gpg >/dev/null
curl -fsSL "https://pkgs.tailscale.com/stable/ubuntu/$(source /etc/os-release && echo "$VERSION_CODENAME").tailscale-keyring.list" | sudo tee /etc/apt/sources.list.d/tailscale.list
...
echo "deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com $(source /etc/os-release && echo "$VERSION_CODENAME") main" | sudo tee /etc/apt/sources.list.d/hashicorp.list
